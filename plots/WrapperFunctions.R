plot_domain_den_by_year <- function(dataset, species, years, title = NULL, print_dataframe = FALSE) {
  
  a <-  getDomainDensity(dataset, species, years = years) %>%
    mutate( SE   = sqrt(var),
            YEAR = as_factor(YEAR))
...
plot_domain_occ_by_year <- function(dataset, species, years, title = NULL, print_dataframe =FALSE) {
  
  a <- getDomainOccurrence(dataset, species, years = years) %>%
    mutate(SE = sqrt(var),
           YEAR = as_factor(YEAR))
...
plot_domain_biomass_by_year <- function(dataset, species, years, title = NULL, print_dataframe = FALSE) {
  
  a <-  getDomainBiomass(dataset, species, years = years) %>%
    mutate( SE   = sqrt(var),
            YEAR = as_factor(YEAR))
